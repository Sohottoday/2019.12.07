...
### 실습1 - while문으로 무한루프 처리

1. WhileLab3 라는 클래스를 생성한다.

2. 0부터 30사이의 난수를 추출한다.

3. 추출된 숫자가 1이면 'A', 2이면 'B', ... 26이면 'Z'를 출력하는데
...
2. ary라는 int타입의 배열 변수를 선언하고 10개의 엘리먼트를 갖는 배열을 생성하여 대입한다.
...
2. 10개의 원소를 갖는 int타입의 배열을 생성한 후에 이 배열의 각각의 원소값으로 1부터 26 사이의 난수를 추출하여 저장한다.
...
for(i = 0 ; i<ten.length ; i++)
    ten[i] = (char)(ran[i]+64); //인덱스가 벗어남. ran[n]이 아니라 ran[i]를 써야함... ran[n]을 써서 한참 답을 못찾고 고민만 함.
	for(i=0 ; i<ten.length ; i++)
        if(i==ten.length-1)
